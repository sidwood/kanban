...
fn recovery_upgrade_retires_legacy_submitted_authority_without_rewriting_results() {
    ...
    connection.execute_batch("UPDATE capabilities SET status='active',settled_at=NULL; UPDATE dispatch_requests SET completed_at=NULL; DROP TABLE run_resume_deliveries; DROP TABLE shell_collapsed_columns; DROP TABLE shell_preferences; DROP TABLE IF EXISTS voided_approvals; ALTER TABLE landing_intents DROP COLUMN command_fingerprint; DELETE FROM schema_migrations WHERE version>=47;").unwrap();
